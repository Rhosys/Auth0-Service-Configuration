{
  getUserProfile: function(accessToken, ctx, cb) {
    const botAccessToken = ctx.token_type === 'bot' ? ctx.access_token || accessToken : (ctx.bot && ctx.bot.bot_access_token);
    if (!botAccessToken) {
      return cb(new Error(`Bot Scope is missing from request. ${JSON.stringify(ctx)}`));
    }

    const userId = ctx.authed_user && ctx.authed_user.id || ctx.user_id;
    request.get({
